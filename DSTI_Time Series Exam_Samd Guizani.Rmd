...
RMSE = function(y_act, y_prd)
  #' Computes Root Mean Squared Error (RMSE) between actual and predicted values. 
  #'
  #' @param y_act Numeric vector. Actual observed values.
  #' @param y_prd Numeric vector. Predicted values.
  #'
  #' @return A single numeric value representing RMSE.
{
...
`Time difference of 5.734455 hours [1] "Cross-validation RMSE: 12.5587524976877"`
...
for (name in names(models_list))
{
     ...
     | grepl("XG", name)) # Use forecast_ML function with ML models 
  {
    ...
    prevision = ts(prevision, start = c(51,1), frequency = 96)
  }
...
for (name in names(models_list))
{
  ...
  {
    prevision = forecast_ML(models_list[[name]], newdata = matrix(newdata_ML,1), horizon)
    prevision = ts(prevision, start = c(8,97), frequency = 7*96)
  }
  else # Use forecast function with ts models
  {
    prevision = forecast(models_list[[name]], h = horizon)
...
prevision = ts(forecast_ML(fit, newdata = matrix(newdata_ML,1), horizon), 
               start = c(52,1), frequency = 96)

df_results = data.frame(Forecast_no_covariate = prevision)
```
...
fit = Arima(data_train[,1], xreg = data_train[,-1], 
            order = c(5,0,0), 
            seasonal = c(0,1,1))
fit |>  summary()
...
-   Cross validation:
    ...
    `Arima_xreg <- function(x, h, xreg, newxreg) { forecast(Arima(x, order=c(5,0,0), seasonal = c(0,1,0), xreg=xreg), xreg=newxreg) }`\
    `e <- tsCV(data_impute[,1], Arima_xreg, h=96, xreg=data_impute[,2], window = 4795)exec_t_end = Sys.time() print(exec_t_end - exec_t_start)print(paste0("Cross-validation RMSE: ", sqrt(mean(e^2, na.rm = TRUE))))`
